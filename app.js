...
function syncControlsFromNearest() {
  ...
  syncEffectTab((l.effect?.type || 'none') === 'wipe' ? 'wipe' : (l.effect?.type || 'none') === 'colorAdjust' ? 'color' : (l.effect?.type || 'none') === 'rasterExtrude' ? 'raster' : 'glow');
...
function bind() {
  ...
  ui.layerEffectType.onchange = () => {
    ...
    syncEffectTab(l.effect.type === 'wipe' ? 'wipe' : l.effect.type === 'colorAdjust' ? 'color' : l.effect.type === 'rasterExtrude' ? 'raster' : 'glow');
    p.updatedAt = Date.now();
    saveProjects();
    draw();
  };
  ...
  document.querySelectorAll('.effect-tab-btn').forEach((btn) => {
    btn.onclick = () => {
      ...
      const p = currentProject(); const l = currentLayer();
      if (!p || !l) return;
      const effectType = tab === 'wipe' ? 'wipe' : tab === 'color' ? 'colorAdjust' : tab === 'raster' ? 'rasterExtrude' : 'glow';
      l.effect = l.effect || { type: 'none', strength: 0.6, glowColor: '#21b8ff', glowHardness: 0.5, glowAlpha: 0.7, reveal: 1, wipeAngle: 0, hue: 0, saturation: 1, brightness: 1, depthAngle: 35, depthSize: 8 };
