...
const ui = {
  ...
  modal: getEl('createProjectModal'),
  closeModalBtn: getEl('closeModalBtn'),
  ratioRow: getEl('ratioRow'),
  modalFps: getEl('modalFps'),
  modalProjectName: getEl('modalProjectName'),
  modalBgColor: getEl('modalBgColor'),
  modalBgHex: getEl('modalBgHex'),
  confirmCreateBtn: getEl('confirmCreateBtn'),
  ...
  easeGraph: getEl('easeGraph')
};

const ctx = ui.preview.getContext('2d');
const graph = ui.easeGraph.getContext('2d');

const state = { projects: [], currentProjectId: null, time: 0, playing: false, startRef: 0, modalRatio: '9:16' };
...
function newProject(config) {
  return {
    id: id(),
    name: config.name || `Project ${state.projects.length + 1}`,
    createdAt: Date.now(),
    updatedAt: Date.now(),
    settings: {
...
function loadProjects() {
  ...
  if (!state.projects.length) {
    state.projects.push(newProject({ name: 'Dự án mặc định', ratio: '9:16', fps: 30, bgColor: '#000000' }));
    saveProjects();
  }
}
...
function setCanvasByRatio(ratio) {
  const r = parseRatio(ratio);
  const baseW = 360;
  ui.preview.width = baseW;
  ui.preview.height = Math.round((baseW * r.h) / r.w);
}
...
function openCreateModal() {
  ...
  ui.ratioRow.querySelectorAll('.ratio-btn').forEach((btn) => btn.classList.toggle('selected', btn.dataset.ratio === '9:16'));
}
...
function renderProjectList() {
  ...
  state.projects.forEach((p) => {
    ...
    row.innerHTML = `<div><strong>${p.name}</strong><br><small>${p.settings.ratio} • ${p.settings.fps} FPS • ${p.settings.bgColor}</small></div>`;
    ...
    del.addEventListener('click', () => {
      ...
      if (!state.projects.length) state.projects.push(newProject({ name: 'Dự án mới', ratio: '9:16', fps: 30, bgColor: '#000000' }));
      saveProjects();
      renderProjectList();
    });
...
function hydrateEditor() {
  ...
  if (!p) return;
  setCanvasByRatio(p.settings.ratio);
...
function drawLayer(l) {
  ...
  if (l.type === 'rect') {
    ...
    const w = l.size;
    const h = (l._img.height / l._img.width) * w;
    ctx.drawImage(l._img, -w / 2, -h / 2, w, h);
  } else {
    ctx.fillStyle = l.color;
    ctx.font = `bold ${l.size}px Inter, sans-serif`;
...
function draw() {
  ...
  for (let x = 0; x <= ui.preview.width; x += 60) {
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(255,255,255,.06)';
    ctx.moveTo(x, 0);
    ctx.lineTo(x, ui.preview.height);
    ctx.stroke();
  }
  for (let y = 0; y <= ui.preview.height; y += 60) {
    ctx.beginPath();
    ctx.strokeStyle = 'rgba(255,255,255,.06)';
    ctx.moveTo(0, y);
    ctx.lineTo(ui.preview.width, y);
    ctx.stroke();
  }
...
function drawEaseGraph() {
  ...
  graph.fillStyle = '#0b0f17';
  graph.fillRect(0, 0, ui.easeGraph.width, ui.easeGraph.height);
  graph.strokeStyle = 'rgba(255,255,255,.25)';
  graph.strokeRect(10, 10, ui.easeGraph.width - 20, ui.easeGraph.height - 20);
  graph.beginPath();
  graph.strokeStyle = '#55d4ff';
  for (let i = 0; i <= 100; i += 1) {
    const t = i / 100;
    const y = bezier(t, 0, cm.p1y, cm.p2y, 1);
    const px = 10 + t * (ui.easeGraph.width - 20);
    const py = (ui.easeGraph.height - 10) - y * (ui.easeGraph.height - 20);
...
function preloadProjectImages() {
  state.projects.forEach((p) => p.layers.forEach((l) => {
    if (l.type === 'image' && l.imageSrc) {
      const img = new Image();
      img.onload = () => { l._img = img; if (p.id === state.currentProjectId) draw(); };
      img.src = l.imageSrc;
    }
  }));
}
...
function bind() {
  ...
  ui.confirmCreateBtn.addEventListener('click', () => {
    const project = newProject({
      name: ui.modalProjectName.value.trim(),
      ratio: state.modalRatio,
      fps: Number(ui.modalFps.value),
      bgColor: ui.modalBgColor.value
    });
  ...
  ui.addImageBtn.addEventListener('click', () => {
    ...
    fr.onload = () => {
      ...
      hydrateEditor();
      const img = new Image();
      img.onload = () => { layer._img = img; draw(); };
      img.src = fr.result;
    };
