...
const ui = {
  ...
  easeTarget: getEl('easeTarget'), easing: getEl('easing'), applyBtn: getEl('applyBtn'), easeGraph: getEl('easeGraph'), scaleQuickInput: getEl('scaleQuickInput'), scaleUpBtn: getEl('scaleUpBtn'), scaleDownBtn: getEl('scaleDownBtn'), frameActionMiniBtn: getEl('frameActionMiniBtn'), easeGraphModeBtn: getEl('easeGraphModeBtn'), effectStrengthSlider: getEl('effectStrengthSlider'), effectStrengthPercent: getEl('effectStrengthPercent'),
...
function syncControlsFromNearest() {
  ...
  if (ui.effectStrengthSlider) ui.effectStrengthSlider.value = String(l.effect?.strength ?? 0.6);
  if (ui.effectStrengthPercent) ui.effectStrengthPercent.value = `${Math.round((l.effect?.strength ?? 0.6) * 100)}%`; 
...
function bind() {
  ...
  ui.layerEffectStrength.oninput = () => {
    ...
    if (ui.effectStrengthSlider) ui.effectStrengthSlider.value = String(l.effect.strength);
    if (ui.effectStrengthPercent) ui.effectStrengthPercent.value = `${Math.round(l.effect.strength * 100)}%`;
    p.updatedAt = Date.now();
    saveProjects();
    draw();
  };
  if (ui.effectStrengthSlider) {
    ui.effectStrengthSlider.oninput = () => {
      ui.layerEffectStrength.value = ui.effectStrengthSlider.value;
      ui.layerEffectStrength.oninput();
    };
  }
