...
const ui = {
  ...
  streakCount: getEl('streakCount'), starBalance: getEl('starBalance'), streakStatus: getEl('streakStatus'), dailyCheckinBtn: getEl('dailyCheckinBtn'), leaderboardList: getEl('leaderboardList')
};
...
function openCloudViewer(item) {
  ...
  if (ui.cloudViewerViews) ui.cloudViewerViews.textContent = `${item.views || 0} views`;
  ...
  recordCloudEngagement(item, 'view');
  updateStudioStats();
}

async function recordCloudEngagement(item, action, type = '') {
  if (!item?.id || String(item.id).startsWith('id-')) return;
  try {
    const response = await fetch(apiUrl(`/api/projects/${encodeURIComponent(item.id)}/${action}`), {
      method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() }, body: JSON.stringify(type ? { type } : {})
    });
    if (!response.ok) return;
    const metrics = await response.json();
    Object.assign(item, metrics);
    if (ui.cloudViewerViews) ui.cloudViewerViews.textContent = `${item.views || 0} views`;
    if (ui.cloudLikeBtn) ui.cloudLikeBtn.textContent = `👍 Like ${item.likes || 0}`;
    if (ui.cloudDislikeBtn) ui.cloudDislikeBtn.textContent = `👎 Dislike ${item.dislikes || 0}`;
  } catch { /* Viewing remains available when the Render API is offline. */ }
}
...
function renderCommunityProfile(profile = null) {
  const p = profile || {};
  if (ui.streakCount) ui.streakCount.textContent = String(p.streak || 0);
  if (ui.starBalance) ui.starBalance.textContent = `${p.stars || 0} ⭐`;
  if (ui.streakStatus) ui.streakStatus.textContent = p.lastCheckin === new Date().toISOString().slice(0, 10)
    ? 'Checked in today — come back tomorrow.'
    : (state.session ? 'Ready for today’s check-in.' : 'Sign in to start your streak.');
  if (ui.dailyCheckinBtn) ui.dailyCheckinBtn.disabled = !state.session || p.lastCheckin === new Date().toISOString().slice(0, 10);
}

async function refreshCommunity() {
  try {
    const [boardResponse, profileResponse] = await Promise.all([
      fetch(apiUrl('/api/community/leaderboard')),
      state.session ? fetch(apiUrl('/api/community/me'), { headers: authHeaders() }) : Promise.resolve(null)
    ]);
    const board = boardResponse.ok ? await boardResponse.json() : [];
    if (ui.leaderboardList) ui.leaderboardList.innerHTML = board.length
      ? board.map((entry, index) => `<li><span>#${index + 1} ${escapeHtml(entry.name || 'Creator')}</span><strong>${entry.stars || 0} ⭐</strong></li>`).join('')
      : '<li><small>No creators have checked in yet.</small></li>';
    if (profileResponse?.ok) renderCommunityProfile(await profileResponse.json());
    else renderCommunityProfile();
  } catch {
    if (ui.leaderboardList) ui.leaderboardList.innerHTML = '<li><small>Community server is offline.</small></li>';
    renderCommunityProfile();
  }
}

async function checkInDaily() {
  if (!state.session) return openGuestModal();
  try {
    const r = await fetch(apiUrl('/api/community/checkin'), { method: 'POST', headers: { ...authHeaders() } });
    const data = await r.json();
    if (!r.ok) throw new Error(data.error || 'Check-in failed');
    renderCommunityProfile(data.profile);
    pushNotification(data.alreadyCheckedIn ? 'Daily streak already protected' : `+${data.reward} Alight Stars`, data.alreadyCheckedIn ? 'You already checked in today.' : `Your streak is now ${data.profile.streak} days.`);
    refreshCommunity();
  } catch (error) { alert(error.message || 'Check-in failed'); }
}
...
async function exportVideoMp4(options = {}) {
  ...
  // 1080p and below remain free; higher render tiers are authorized by the server.
  if (exportResolution >= 2160) {
    if (!state.session) { openGuestModal(); return; }
    try {
      const response = await fetch(apiUrl('/api/economy/export'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ resolution: exportResolution })
      });
      const receipt = await response.json();
      if (!response.ok) throw new Error(receipt.error || 'Export credit unavailable');
      renderCommunityProfile(receipt.profile);
      pushNotification(receipt.happyHour ? 'Happy Hour 8K unlocked!' : 'Export tier unlocked', receipt.cost ? `${receipt.cost} ⭐ used for this export.` : 'This export is free.');
    } catch (error) {
      alert(error.message || 'Unable to charge export tier.');
      return;
    }
  }
  const quality = p.settings.exportQuality || 'medium';
...
function bind() {
  ...
  if (ui.dailyCheckinBtn) ui.dailyCheckinBtn.onclick = checkInDaily;
  ...
  if (ui.cloudLikeBtn) ui.cloudLikeBtn.onclick = () => { if (state.activeCloudItem) recordCloudEngagement(state.activeCloudItem, 'reaction', 'like'); };
  if (ui.cloudDislikeBtn) ui.cloudDislikeBtn.onclick = () => { if (state.activeCloudItem) recordCloudEngagement(state.activeCloudItem, 'reaction', 'dislike'); };
