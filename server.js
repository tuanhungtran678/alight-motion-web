...
function profileFor(user) {
  ...
  let profile = profiles.find((item) => item.userId === user.id);
  if (!profile) {
    profile = { userId: user.id, name: user.name || user.email.split('@')[0], stars: 0, streak: 0, lastCheckin: '', updatedAt: Date.now() };
...
function exportCost(resolution) {
  if (resolution >= 4320) return 1500;
  if (resolution >= 2160) return 750;
  return 0;
}

function isHappyHour() {
  const now = new Date();
  return now.getUTCHours() === 22 && now.getUTCMinutes() >= 39;
}

function readEncodedHeader(value, fallback = '') {
  ...
  if (!uid && emailHeader) {
    ...
    if (!user) {
      user = {
        ...
        email: emailHeader,
        name: readEncodedHeader(req.headers['x-demo-user-name'], emailHeader.split('@')[0]),
        provider: readEncodedHeader(req.headers['x-demo-user-provider'], 'Firebase')
      };
      users.push(user);
...
http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/community/leaderboard' && req.method === 'GET') {
    const profiles = readJson(PROFILES_FILE, [])
      .sort((a, b) => (b.stars - a.stars) || (b.streak - a.streak))
      .slice(0, 20)
      .map((p) => ({ name: p.name, stars: p.stars, streak: p.streak }));
    return send(res, 200, profiles);
  }

  if (req.url === '/api/community/me' && req.method === 'GET') {
    const user = getUserFromReq(req);
    if (!user) return send(res, 401, { error: 'Unauthorized' });
    return send(res, 200, profileFor(user));
  }

  if (req.url === '/api/community/checkin' && req.method === 'POST') {
    const user = getUserFromReq(req);
    if (!user) return send(res, 401, { error: 'Unauthorized' });
    const profile = profileFor(user);
    const today = todayKey();
    if (profile.lastCheckin === today) return send(res, 200, { alreadyCheckedIn: true, reward: 0, profile });
    const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
    profile.streak = profile.lastCheckin === yesterday ? Number(profile.streak || 0) + 1 : 1;
    profile.stars = Number(profile.stars || 0) + 25;
    profile.lastCheckin = today;
    profile.updatedAt = Date.now();
    saveProfile(profile);
    return send(res, 200, { alreadyCheckedIn: false, reward: 25, profile });
  }

  if (req.url === '/api/economy/export' && req.method === 'POST') {
    const user = getUserFromReq(req);
    if (!user) return send(res, 401, { error: 'Unauthorized' });
    try {
      const body = await parseBody(req);
      const resolution = Number(body.resolution || 1080);
      const freeHappyHour = resolution >= 4320 && isHappyHour();
      const cost = freeHappyHour ? 0 : exportCost(resolution);
      const profile = profileFor(user);
      if (profile.stars < cost) return send(res, 402, { error: `Need ${cost} Alight Stars for this export.`, cost, profile });
      profile.stars -= cost;
      profile.updatedAt = Date.now();
      saveProfile(profile);
      return send(res, 200, { ok: true, cost, happyHour: freeHappyHour, profile });
    } catch { return send(res, 400, { error: 'Bad payload' }); }
  }
  ...
  if (req.url && /^\/api\/projects\/[^/]+\/(view|reaction)$/.test(req.url) && req.method === 'POST') {
    const [, id, action] = req.url.match(/^\/api\/projects\/([^/]+)\/(view|reaction)$/) || [];
    const db = readJson(DB_FILE, []);
    const item = db.find((project) => project.id === decodeURIComponent(id));
    if (!item || !item.shared) return send(res, 404, { error: 'Not found' });
    try {
      const body = await parseBody(req);
      if (action === 'view') item.views = Number(item.views || 0) + 1;
      if (action === 'reaction') {
        if (body.type === 'like') item.likes = Number(item.likes || 0) + 1;
        else if (body.type === 'dislike') item.dislikes = Number(item.dislikes || 0) + 1;
        else return send(res, 400, { error: 'Unsupported reaction' });
      }
      writeJson(DB_FILE, db);
      return send(res, 200, { id: item.id, views: item.views || 0, likes: item.likes || 0, dislikes: item.dislikes || 0 });
    } catch { return send(res, 400, { error: 'Bad payload' }); }
  }
