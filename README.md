...
## Community economy (server-authoritative)

The Node backend now keeps a separate `profiles.json` data store for every authenticated profile. It exposes a daily check-in endpoint that uses the server UTC date (rather than browser time) to protect streaks from local-clock spoofing. A successful first check-in awards **25 Alight Stars**; consecutive days increase the streak. The Home screen reads the current profile balance and a server leaderboard.

Render tiers are verified before recording starts: **1080p and lower are free**, **4K costs 750 Stars**, and **8K costs 1,500 Stars**. At **22:39–22:59 UTC**, 8K is temporarily free as Happy Hour. This is a server-side debit, so moving a device clock cannot obtain a tier for free.
...
- `POST /api/economy/export`

`profiles.json` is runtime data and is intentionally ignored by Git. For Render persistence, attach a persistent disk or replace these JSON stores with a managed database; Render's ephemeral filesystem is not a durable database.
